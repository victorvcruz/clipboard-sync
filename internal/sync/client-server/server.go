...
func (ss *Server) startMemoryCleanup() {
	...
	log.Printf("Server memory cleanup routine started")

	for {
		select {
		case <-ss.cleanupStop:
			log.Printf("Server memory cleanup routine stopped")
			return
		case <-ticker.C:
...
func (ss *Server) cleanupOldReceivedEntries() {
	...
	if removedCount > 0 {
		log.Printf(
			"Server cleanup: removed %d old source entries, %d remaining",
			removedCount,
			finalCount,
		)
	}
	...
	if finalCount > 500 {
		log.Printf(
			"Warning: Server has %d source entries, performing emergency cleanup",
			finalCount,
		)
...
func (ss *Server) emergencyCleanupReceived() {
	...
	log.Printf(
		"Server emergency cleanup: removed %d source entries, kept %d most recent",
		removedCount,
		keepCount,
	)
}
